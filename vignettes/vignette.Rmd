...
knitr::include_graphics("../man/figures/Data_layout2.png", dpi = 600, error = F)
```

`r cat("")`\
To read data in custom format, run:

```{r read-custom, echo = TRUE, eval = F}
...
```{r load-data, echo = T, eval = TRUE, results='hide', message=FALSE, error=FALSE, warning=FALSE}
...
```{r head-input, echo = T, eval = TRUE}
head(input$expdesign)
```

We can plot the raw data:

```{r raw-plot, fig.width=9, fig.height=9, echo = T, eval = TRUE, results='hide', message=FALSE, error=FALSE, warning=FALSE}
plot.grofit(input, data.type = "raw", log.y = FALSE, x.lim = c(NA, 32), names = "KT2440")
```

## Run Workflow

To perform a complete growth profiling of all samples in the input dataset, we call the `growth.workflow()` function on our `input` object:

```{r run-default-workflow}
growth.workflow(grodata = input)
```
