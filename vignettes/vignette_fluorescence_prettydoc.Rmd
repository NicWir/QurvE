...
For a general introduction to the `QurvE` package, see the vignette *Quantitiative Fluorescence Curve Evaluation with Package `QurvE`*.

Fluorescent reporter proteins are widely used to study the mechanisms of gene regulation or to develop biosensors that allow the detection of chemical compounds and provide insights into the intracellular environment.
...
grodata <- read_data(data.density = system.file('2-FMA_toxicity.csv', package = 'QurvE'), csvsep = ";" )
```
