...
if(any(c("a", "l") %in% control$fit.opt)){
  ...
  if(all(c('pdf', 'html') %in% format)){
    if (knitr::is_latex_output()) {
      foreach(i = 1:length(grofit[["gcFit"]][["gcFittedLinear"]]),
              .packages = "ggplot2") %dopar% {
                if(grofit[["gcFit"]][["gcFittedLinear"]][[i]][["fitFlag"]] == TRUE){
                  pdf(file=paste0(plotdir, "/PlotLinear_", gsub("∙", "_", gsub("\\?", "_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit[["gcFit"]][["gcFittedLinear"]])[i]))))), ".pdf"), width = 6, height = 4.5);   plot.ggLinear();   invisible(dev.off())
                }
              }
    }
  } else{
    foreach(i = 1:length(grofit[["gcFit"]][["gcFittedLinear"]]),
              .packages = "ggplot2") %dopar% {
                if(grofit[["gcFit"]][["gcFittedLinear"]][[i]][["fitFlag"]] == TRUE){
                  pdf(file=paste0(plotdir, "/PlotLinear_", gsub("∙", "_", gsub("%", "_", gsub("\\?", "_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit[["gcFit"]][["gcFittedLinear"]])[i])))))), ".pdf"), width = 6, height = 4.5);   plot.ggLinear();   invisible(dev.off())
                }
              }
  } 
  ...
  Files_list <- list.files(path = plotdir, 
                                pattern = "^PlotLinear_",
                                full.names = TRUE)
  sort.ndx <- match(gsub("∙", "_", gsub("%", "_", gsub("\\?", "_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit$gcFit$gcFittedLinear))))))), gsub(".+PlotLinear_","", gsub(".pdf", "", Files_list)))
...
if(any(c("a", "b", "m") %in% control$fit.opt)){
  ...
  if(all(c('pdf', 'html') %in% format)){
    if (knitr::is_latex_output()) {
      foreach(i = 1:  1:length(grofit[["gcFit"]][["gcFittedModels"]]),
        .packages = "ggplot2") %dopar% {
          if(grofit[["gcFit"]][["gcFittedModels"]][[i]][["fitFlag"]] == TRUE){
            pdf(file=paste0(plotdir, "/PlotModel_", gsub("∙", "_", gsub("\\?", "_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit[["gcFit"]][["gcFittedModels"]])[i]))))), ".pdf"), width = 6, height = 4.5);   plot.ggModel();   invisible(dev.off())
          }
        }
    }
  } else{
    foreach(i = 1:  1:length(grofit[["gcFit"]][["gcFittedModels"]]),
        .packages = "ggplot2") %dopar% {
          if(grofit[["gcFit"]][["gcFittedModels"]][[i]][["fitFlag"]] == TRUE){
            pdf(file=paste0(plotdir, "/PlotModel_", gsub("∙", "_", gsub("%", "_", gsub("\\?", "_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit[["gcFit"]][["gcFittedModels"]])[i])))))), ".pdf"), width = 6, height = 4.5);   plot.ggModel();   invisible(dev.off())
          }
        }
  }
  ...
  Files_list <- list.files(path = plotdir, 
                                pattern = "^PlotModel_",
                                full.names = TRUE)
  sort.ndx <- match(gsub("∙", "_", gsub("%", "_", gsub("\\?", "_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit$gcFit$gcFittedModels))))))), gsub(".+PlotModel_","", gsub(".pdf", "", Files_list)))
  sort.ndx <- sort.ndx[!is.na(sort.ndx)]
knitr::include_graphics(Files_list[sort.ndx], dpi = 300, error = F)
}
```
...
if("all" %in% mean.grp){
  if("s" %in% grofit$control$fit.opt){
    ...
    plot.grofit(grofit, data.type = "raw", basesize = 15, export = export, log.y = T, mean = T, out.dir = paste0(wd, "/Plots"))
  }
  ...
  for(i in 1:length(mean.grp)){
    if("s" %in% grofit$control$fit.opt){
      ...
      plot.grofit(grofit, data.type = "raw", names = unlist(mean.grp[i]), basesize = 15, export = export, log.y = T, mean = T, out.dir = paste0(wd, "/Plots"))
    }
  }
}
if(!all(is.na(mean.conc))){
  for(i in 1:length(mean.conc)){
    ...
    if(length(grep(mean.conc[i], str_extract(names(grofit$gcFit$gcFittedSplines), "[:alnum:]+$")))>0){
      ...
      plot.grofit(grofit, data.type = "raw", conc = unlist(mean.conc[i]), basesize = 15, export = export, log.y = T, mean = T, out.dir = paste0(wd, "/Plots"))
    }
  }
}
```
