...
print(plot.parameters(grofit, param = "mu.linfit"))
print(plot.parameters(grofit, param = "lambda.linfit"))
print(plot.parameters(grofit, param = "dY.linfit"))
```
...
print(plot.parameters(grofit, param = "dY.model"))
```
  ...
  print(plot(grofit, basesize = 15))
    ...
    for(i in 1:length(mean.grp)){
      if(length(grep(mean.grp[i], as.character(names(grofit$gcFit$gcFittedSplines))))>0){
        print(plot(grofit, names = unlist(mean.grp[i]), basesize = 15))
      }
    }
}
if(!all(is.na(mean.conc))){
  for(i in 1:length(mean.conc)){
    if(length(grep(mean.conc[i], str_extract(names(grofit$gcFit$gcFittedSplines), "[:alnum:]+$")))>0){
      print(plot(grofit, conc = unlist(mean.conc[i]), basesize = 15))
    }
  }
}
```
