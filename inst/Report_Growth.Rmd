...
plot.parameters(grofit, param = "mu.linfit", export = export)
plot.parameters(grofit, param = "lambda.linfit", export = export)
plot.parameters(grofit, param = "dY.linfit", export = export)
```
...
print(plot.parameters(grofit, param = "mu.model"))
print(plot.parameters(grofit, param = "lambda.model"))
```
...
print(plot.parameters(grofit, param = "mu.spline", export = export))
print(plot.parameters(grofit, param = "lambda.spline", export = export))
print(plot.parameters(grofit, param = "dY.spline", export = export))
```
\clearpage
...
if (knitr::is_latex_output()) {
    foreach(i = 1:  1:length(grofit[["gcFit"]][["gcFittedSplines"]]),
        .packages = "ggplot2") %dopar% {
          if(grofit[["gcFit"]][["gcFittedSplines"]][[i]][["fitFlag"]] == TRUE){
            pdf(file=paste0(plotdir, "/PlotSpline_", gsub(" \\| ", "_", names(grofit[["gcFit"]][["gcFittedSplines"]])[i]), ".pdf"), ".pdf"));   plot.ggSpline();   invisible(dev.off())
          }
        }
  }
  Files_list <- list.files(path = paste0(str_replace_all(tempdir(), "\\\\", "/"), "/Plots/"), 
...
for(i in 1:length(grofit$drFit$drBootSplines)){
  if(grofit$drFit$drBootSplines[[i]]$ bootFlag == TRUE){
    plot.drBootSpline(grofit$drFit$drBootSplines[[i]], width = 6, height = 4.5, export = export)
  }
}
```
...
if("all" %in% mean.grp){
  if(length(grofit$gcFit$gcFittedSplines[[1]]$spline)>1){
    print(plot(grofit, basesize = 15, export = export, log.y = T, mean = T))
  }
    ...
    for(i in 1:length(mean.grp)){
      if(length(grep(mean.grp[i], as.character(names(grofit$gcFit$gcFittedSplines))))>0){
        print(plot(grofit, names = unlist(mean.grp[i]), basesize = 15, export = export, log.y = T, mean = T))
      }
    }
}
if(!all(is.na(mean.conc))){
  for(i in 1:length(mean.conc)){
    if(length(grep(mean.conc[i], str_extract(names(grofit$gcFit$gcFittedSplines), "[:alnum:]+$")))>0){
      print(plot(grofit, conc = unlist(mean.conc[i]), basesize = 15, export = export, log.y = T, mean = T))
    }
  }
}
```
