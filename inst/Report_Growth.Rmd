...
if(any(c("a", "l") %in% control$fit.opt)){
  ...
  Files_list <- list.files(path = plotdir, 
                                pattern = "^PlotLinear_",
                                full.names = TRUE)
  sort.ndx <- match(gsub("\\?", "_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit$gcFit$gcFittedModels))))), gsub(".+PlotLinear_","", gsub(".pdf", "", Files_list)))
...
if(any(c("a", "b", "m") %in% control$fit.opt)){
  ...
  Files_list <- list.files(path = plotdir, 
                                pattern = "^PlotModel_",
                                full.names = TRUE)
  sort.ndx <- match(gsub("\\?", "_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit$gcFit$gcFittedModels))))), gsub(".+PlotModel_","", gsub(".pdf", "", Files_list)))
  sort.ndx <- sort.ndx[!is.na(sort.ndx)]
knitr::include_graphics(Files_list[sort.ndx], dpi = 300, error = F)
}
```
  ...
  if(all(c('pdf', 'html') %in% format)){
    if (knitr::is_latex_output()) {
      foreach(i = 1:length(grofit[["gcFit"]][["gcFittedSplines"]]),
            .packages = "ggplot2") %dopar% {
              if (grofit[["gcFit"]][["gcFittedSplines"]][[i]][["fitFlag"]] == TRUE) {
                pdf(file = paste0(plotdir, "/PlotSpline_", gsub("\\?", "_", gsub(" ", "_", gsub(
