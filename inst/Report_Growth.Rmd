...
if(any(c("a", "l") %in% control$fit.opt)){
  ...
  if (knitr::is_latex_output()) {
    foreach(i = 1:length(grofit[["gcFit"]][["gcFittedLinear"]]),
        .packages = "ggplot2") %dopar% {
          if(grofit[["gcFit"]][["gcFittedLinear"]][[i]][["fitFlag"]] == TRUE){
            pdf(file=paste0(plotdir, "/PlotLinear_", gsub(" \\| ", "_", names(grofit[["gcFit"]][["gcFittedLinear"]])[i]), ".pdf"), width = 6, height = 4.5);   plot.ggLinear();   invisible(dev.off())
          }
        }
  }
  Files_list <- list.files(path = plotdir, 
                                pattern = "^PlotLinear_",
                                full.names = TRUE)
  sort.ndx <- match(gsub(" \\| ", "_", names(grofit$gcFit$gcFittedModels)), gsub(".+PlotLinear_","", gsub(".pdf", "", Files_list)))
...
if(any(c("a", "b", "m") %in% control$fit.opt)){
  ...
  if (knitr::is_latex_output()) {
    foreach(i = 1:  1:length(grofit[["gcFit"]][["gcFittedModels"]]),
        .packages = "ggplot2") %dopar% {
          if(grofit[["gcFit"]][["gcFittedModels"]][[i]][["fitFlag"]] == TRUE){
            pdf(file=paste0(plotdir, "/PlotModel_", gsub(" \\| ", "_", names(grofit[["gcFit"]][["gcFittedModels"]])[i]), ".pdf"), width = 6, height = 4.5);   plot.ggModel();   invisible(dev.off())
          }
        }
  }
  ...
  Files_list <- list.files(path = plotdir, 
                                pattern = "^PlotModel_",
                                full.names = TRUE)
  sort.ndx <- match(gsub(" \\| ", "_", names(grofit$gcFit$gcFittedModels)), gsub(".+PlotModel_","", gsub(".pdf", "", Files_list)))
  sort.ndx <- sort.ndx[!is.na(sort.ndx)]
knitr::include_graphics(Files_list[sort.ndx], dpi = 300, error = F)
}
```
...
if (knitr::is_latex_output()) {
    foreach(i = 1:  1:length(grofit[["gcFit"]][["gcFittedSplines"]]),
        .packages = "ggplot2") %dopar% {
          if(grofit[["gcFit"]][["gcFittedSplines"]][[i]][["fitFlag"]] == TRUE){
            pdf(file=paste0(plotdir, "/PlotSpline_", gsub(" \\| ", "_", names(grofit[["gcFit"]][["gcFittedSplines"]])[i]), ".pdf"));   plot.ggSpline();   invisible(dev.off())
          }
        }
  }
  Files_list <- list.files(path = plotdir, 
                                pattern = "^PlotSpline_",
                                full.names = TRUE)
    sort.ndx <- match(gsub(" \\| ", "_", names(grofit$gcFit$gcFittedModels)), gsub(".+PlotSpline_","", gsub(".pdf", "", Files_list)))
  sort.ndx <- sort.ndx[!is.na(sort.ndx)]
knitr::include_graphics(Files_list[sort.ndx], dpi = 300, error = F)
}
```
