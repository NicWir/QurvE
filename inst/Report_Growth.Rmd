...
`r if(any(c("a", "l") %in% control$fit.opt)){paste0("* sliding window size in linear regression:")}` `r if(any(c("a", "l") %in% control$fit.opt)){colFmt(ifelse(!is.null(lin.h), lin.h, "automated calculation"), 'NavyBlue')}`
`r if(any(c("a", "l") %in% control$fit.opt)){paste0("* R2 threshold for linear regression:")}` `r if(any(c("a", "l") %in% control$fit.opt)){colFmt(lin.R2, 'NavyBlue')}`
`r if(any(c("a", "l") %in% control$fit.opt)){paste0("* RSD threshold for linear regression:")}` `r if(any(c("a", "l") %in% control$fit.opt)){colFmt(lin.RSD, 'NavyBlue')}`
`r if(any(c("a", "l") %in% control$fit.opt)){paste0("* Relative ΔY threshold for linear regression:")}` `r if(any(c("a", "l") %in% control$fit.opt)){colFmt(lin.dY, 'NavyBlue')}`
...
if(any(c("a", "b", "m") %in% control$fit.opt)){
    ...
    table_model <- suppressWarnings(data.frame("Sample|<br>Replicate|<br>Conc." = paste(res.table.gc$TestId, res.table.gc$AddId, res.table.gc$concentration, sep = "|"),
    ...
    if ( "gompertz.exp" %in% res.table.gc$used.model ){
      table_model <- suppressWarnings(cbind(table_model, data.frame("t~shift~" = round(as.numeric(res.table.gc$parameter_t_shift.model), 3), stringsAsFactors = F, check.names = F)))
    }
    table_model <- as.data.frame(apply(table_model, 2, function(x) str_replace_all(x, "NA ± NA", "")))
    table_model[is.na(table_model)] <- ""
    knit_table(table_model, caption = "Parametric Fit", longtable = TRUE) %>% column_spec(column=1, width = "4.0cm")
    }
}
```
...
if(any(c("a", "l") %in% control$fit.opt)){
  ...
  if (knitr::is_latex_output()) {
    foreach(i = 1:length(grofit[["gcFit"]][["gcFittedLinear"]]),
        .packages = "ggplot2") %dopar% {
          if(grofit[["gcFit"]][["gcFittedLinear"]][[i]][["fitFlag"]] == TRUE){
            pdf(file=paste0(plotdir, "/PlotLinear_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit[["gcFit"]][["gcFittedLinear"]])[i]))), ".pdf"), width = 6, height = 4.5);   plot.ggLinear();   invisible(dev.off())
          }
        }
  }
  Files_list <- list.files(path = plotdir, 
                                pattern = "^PlotLinear_",
                                full.names = TRUE)
  sort.ndx <- match(gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit$gcFit$gcFittedModels)))), gsub(".+PlotLinear_","", gsub(".pdf", "", Files_list)))
...
if(any(c("a", "b", "m") %in% control$fit.opt)){
  ...
  plotdir <- ifelse(export == TRUE, wd.plots, paste0(str_replace_all(tempdir(), "\\\\", "/"),"/Plots"))
  if (knitr::is_latex_output()) {
    foreach(i = 1:  1:length(grofit[["gcFit"]][["gcFittedModels"]]),
        .packages = "ggplot2") %dopar% {
          if(grofit[["gcFit"]][["gcFittedModels"]][[i]][["fitFlag"]] == TRUE){
            pdf(file=paste0(plotdir, "/PlotModel_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit[["gcFit"]][["gcFittedModels"]])[i]))), ".pdf"), width = 6, height = 4.5);   plot.ggModel();   invisible(dev.off())
          }
        }
  }
  ...
  Files_list <- list.files(path = plotdir, 
                                pattern = "^PlotModel_",
                                full.names = TRUE)
  sort.ndx <- match(gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit$gcFit$gcFittedModels)))), gsub(".+PlotModel_","", gsub(".pdf", "", Files_list)))
  sort.ndx <- sort.ndx[!is.na(sort.ndx)]
knitr::include_graphics(Files_list[sort.ndx], dpi = 300, error = F)
}
```
...
plot.ggSpline   <- function(){
  if(grofit[["gcFit"]][["gcFittedSplines"]][[i]][["fitFlag"]] == TRUE){
      ...
      # /// add tangent at maximum slope
       ...
       bla <- ifelse(x$control$log.y.spline == TRUE, exp(coef["b.tangent"][[1]])*x[["data.in"]][1], coef["b.tangent"][[1]])*exp(mu*time)
...
plotdir <- ifelse(export == TRUE, wd.plots, paste0(str_replace_all(tempdir(), "\\\\", "/"),"/Plots"))
if (knitr::is_latex_output()) {
    foreach(i = 1:  1:length(grofit[["gcFit"]][["gcFittedSplines"]]),
        .packages = "ggplot2") %dopar% {
          if(grofit[["gcFit"]][["gcFittedSplines"]][[i]][["fitFlag"]] == TRUE){
            pdf(file=paste0(plotdir, "/PlotSpline_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit[["gcFit"]][["gcFittedSplines"]])[i]))), ".pdf"));   plot.ggSpline();   invisible(dev.off())
          }
        }
  }
  Files_list <- list.files(path = plotdir, 
                                pattern = "^PlotSpline_",
                                full.names = TRUE)
    sort.ndx <- match(gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(grofit$gcFit$gcFittedModels)))), gsub(".+PlotSpline_","", gsub(".pdf", "", Files_list)))
  sort.ndx <- sort.ndx[!is.na(sort.ndx)]
knitr::include_graphics(Files_list[sort.ndx], dpi = 300, error = F)
}
```
...
```{r ec50_spline_plots, fig.width=7, fig.height=5, echo=F, warning=F, message=F, fig.align='center', eval = ec50 && length(grofit[["drFit"]][["drFittedSplines"]]) > 0}
plot.drFit(grofit$drFit, export = export)
```

```{r ec50.boot_summary, echo=F, warning=F, message=F, results='asis', eval = ec50 && length(grofit[["drFit"]][["drFittedSplines"]]) > 0 && (is.numeric(grofit$control$nboot.dr) && grofit$control$nboot.dr > 0)}
...
if("all" %in% mean.grp){
  if(length(grofit$gcFit$gcFittedSplines[[1]]$spline)>1){
    plot.grofit(grofit, basesize = 15, export = export, log.y = T, mean = T, out.dir = paste0(wd, "/Plots"))
  }
    ...
    for(i in 1:length(mean.grp)){
      if(length(grep(mean.grp[i], as.character(names(grofit$gcFit$gcFittedSplines))))>0){
        plot.grofit(grofit, names = unlist(mean.grp[i]), basesize = 15, export = export, log.y = T, mean = T, out.dir = paste0(wd, "/Plots"))
      }
    }
}
if(!all(is.na(mean.conc))){
  for(i in 1:length(mean.conc)){
    if(length(grep(mean.conc[i], str_extract(names(grofit$gcFit$gcFittedSplines), "[:alnum:]+$")))>0){
      plot.grofit(grofit, conc = unlist(mean.conc[i]), basesize = 15, export = export, log.y = T, mean = T, out.dir = paste0(wd, "/Plots"))
    }
  }
}
```
