...
* minimum time considered for exponential and spline fits (t0): `r colFmt(control$t0, 'NavyBlue')`
...
if(any(c("a", "l") %in% control$fit.opt)){
  ...
  table_linear <- data.frame("Sample|Replicate|Conc." = paste(res.table.gc$TestId, res.table.gc$AddId, res.table.gc$concentration, sep = "|"),
                             ...
                             "µ~max~" = ifelse(res.table.gc$mu.linfit==0 | is.na(res.table.gc$mu.linfit), "", ifelse(is.na(res.table.gc$mu2.linfit), round(as.numeric(res.table.gc$mu.linfit), 3), paste0("**", round(as.numeric(res.table.gc$mu.linfit), 3), "**", " (", round(as.numeric(res.table.gc$mu2.linfit), 3), ")"))),
                             
                             "t~D~" = ifelse(res.table.gc$mu.linfit==0 | is.na(res.table.gc$mu.linfit), "",  ifelse(is.na(res.table.gc$mu2.linfit), round(log(2)/as.numeric(res.table.gc$mu.linfit), 2), paste0("**", round(log(2)/as.numeric(res.table.gc$mu.linfit), 2), "**", " (", round(log(2)/as.numeric(res.table.gc$mu2.linfit), 2), ")"))),
                             ...
                             "y~max~" = round(as.numeric(res.table.gc$A.linfit), 3),
                             
                             
                             "t~start~<br>(µ~max~)" = ifelse(is.na(res.table.gc$mu2.linfit), round(as.numeric(res.table.gc$tmu.start.linfit), 2), paste0("**", round(as.numeric(res.table.gc$tmu.start.linfit), 2), "**", " (", round(as.numeric(res.table.gc$tmu2.start.linfit), 2), ")")),
                             
                             
                             "t~end~<br>(µ~max~)" = ifelse(is.na(res.table.gc$mu2.linfit), round(as.numeric(res.table.gc$tmu.end.linfit), 2), paste0("**", round(as.numeric(res.table.gc$tmu.end.linfit), 2), "**", " (", round(as.numeric(res.table.gc$tmu2.end.linfit), 2), ")")),
                             
                             
                             
                             "R^2^<br>(linear fit)" = ifelse(is.na(res.table.gc$mu2.linfit), round(as.numeric(res.table.gc$r2mu.linfit), 3), paste0("**", round(as.numeric(res.table.gc$r2mu.linfit), 3), "**", " (", round(as.numeric(res.table.gc$r2mu2.linfit), 3), ")")),
                             
                             
                             stringsAsFactors = F, check.names = F)
  table_linear <- as.data.frame(apply(table_linear, 2, function(x) str_replace_all(x, "NA ± NA", "")))
  table_linear[is.na(table_linear)] <- ""
  knit_table(table_linear, caption = ifelse(control$biphasic, "Exponential Fit (Values in parentheses indicate parameters for secondary growth phase)", "Exponential Fit"), longtable = TRUE) %>% column_spec(column=1, width = "4cm")
    }
}
```
...
if(any(c("a", "b", "s") %in% control$fit.opt)){
  if(!all(is.na(res.table.gc$lambda.spline))){
    table_spline <- data.frame("Sample|Replicate|Conc." = paste(res.table.gc$TestId, res.table.gc$AddId, res.table.gc$concentration, sep = "|"),
                             ...
                             "µ~max~" = ifelse(res.table.gc$mu.spline==0 | is.na(res.table.gc$mu.spline), "", ifelse(is.na(res.table.gc$mu2.spline), round(as.numeric(res.table.gc$mu.spline), 3), paste0("**", round(as.numeric(res.table.gc$mu.spline), 3), "**", " (", round(as.numeric(res.table.gc$mu2.spline), 3), ")"))),
                             
                             "t~D~" = ifelse(res.table.gc$mu.spline==0 | is.na(res.table.gc$mu.spline), "",  ifelse(is.na(res.table.gc$mu2.spline), round(log(2)/as.numeric(res.table.gc$mu.spline), 2), paste0("**", round(log(2)/as.numeric(res.table.gc$mu.spline), 2), "**", " (", round(log(2)/as.numeric(res.table.gc$mu2.spline), 2), ")"))),
                             
                             "λ" = round(as.numeric(res.table.gc$lambda.spline), 2), 
                             
                             "y~max~" = round(as.numeric(res.table.gc$A.spline), 3),
                             
                             "Δy" = round(as.numeric(res.table.gc$dY.spline), 3),
                             
                             "t~max~" = ifelse(is.na(res.table.gc$mu2.spline), round(as.numeric(res.table.gc$tmax.spline), 2), paste0("**", round(as.numeric(res.table.gc$tmax.spline), 2), "**", " (", round(as.numeric(res.table.gc$tmax2.spline), 2), ")")),
  ...
  knit_table(table_spline, caption = ifelse(control$biphasic, "Smooth Spline Fit (Values in parentheses indicate parameters for secondary growth phase)", "Smooth Spline Fit"), longtable = TRUE) %>% column_spec(column=1, width = "4.0cm")
  }
}
```
...
if(any(c("a", "b", "s") %in% control$fit.opt)) {
  ...
  if (knitr::is_latex_output()) {
    foreach(i = 1:length(grofit[["gcFit"]][["gcFittedSplines"]]),
            .packages = "ggplot2") %dopar% {
              if (grofit[["gcFit"]][["gcFittedSplines"]][[i]][["fitFlag"]] == TRUE) {
                pdf(file = paste0(plotdir, "/PlotSpline_", gsub(" ", "_", gsub(
  ...
  Files_list <- list.files(path = plotdir,
                           ...
                           full.names = TRUE)
  sort.ndx <-
    match(gsub(" ", "_", gsub("\\/", "-", gsub(
      " \\| ", "_", names(grofit$gcFit$gcFittedModels)
...
for(i in 1:length(grofit$drFit$drFittedSplines)){
  ec50.df <- rbind.fill(ec50.df, data.frame("Condition" = names(grofit$drFit$drFittedSplines)[i],
