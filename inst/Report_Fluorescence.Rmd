...
if(any(c("s") %in% control$fit.opt)) {
  ...
  plot.ggSpline   <- function() {
    if (flFitRes[["flFit"]][["flFittedSplines"]][[i]][["fitFlag"]] == TRUE) {
      x <- flFitRes[["flFit"]][["flFittedSplines"]][[i]]
      plot.flFitSpline2(x)
    }
  }
  plotdir <-
            ...
            .packages = "ggplot2") %dopar% {
              if (length(grep("^NA ", names(flFitRes[["flFit"]][["flFittedSplines"]])[i]))==0 && flFitRes[["flFit"]][["flFittedSplines"]][[i]][["fitFlag"]] == TRUE) {
                suppressWarnings(pdf(file = paste0(plotdir, "/PlotSpline_", gsub("∙", "_", gsub("%", "_", gsub("\\?", "_", gsub(" ", "_", gsub(
                  "\\/", "-", gsub(" \\| ", "_", names(flFitRes[["flFit"]][["flFittedSplines"]])[i])
                ))))), ".pdf")))
                suppressWarnings(
                  plot.ggSpline()
                )
...
if("all" %in% mean.grp){
  if(length(flFitRes$flFit$flFittedSplines[[1]]$spline)>1){
    ...
    suppressWarnings(
      plot.flFitRes(flFitRes, basesize = 15, export = export, log.y = logy, mean = T, out.dir = paste0(wd, "/Plots"), legend.position = "bottom", legend.ncol = 4)
    )
  }
  ...
  for(i in 1:length(mean.grp)){
    if(length(grep(mean.grp[i], as.character(names(flFitRes$flFit$flFittedSplines))))>0){
      suppressWarnings(
        plot.flFitRes(flFitRes, names = unlist(mean.grp[i]), basesize = 15, export = export, log.y = T, mean = T, out.dir = paste0(wd, "/Plots"), legend.position = "bottom", legend.ncol = 4)
      )
    }
  }
}
if(!all(is.na(mean.conc))){
  for(i in 1:length(mean.conc)){
    if(length(grep(mean.conc[i], str_extract(names(flFitRes$flFit$flFittedSplines), "[:alnum:]+$")))>0){
      suppressWarnings(
        plot.flFitRes(flFitRes, conc = unlist(mean.conc[i]), basesize = 15, export = export, log.y = T, mean = T, out.dir = paste0(wd, "/Plots"), legend.position = "bottom", legend.ncol = 4)
      )
    }
  }
}
