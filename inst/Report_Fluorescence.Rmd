...
\pagebreak
## `r if(length(flFitRes$data$density)>1 && length(flFitRes$data$fluorescence1)>1){"Graphical overview -  Fluorescence 1"}`
```{r dual_plot_fl1, fig.height = 24, fig.width = 20, echo=F, warning=F, message=F, results='asis', fig.align='default', eval = length(flFitRes$data$density)>1 && length(flFitRes$data$fluorescence1)>1}
suppressWarnings(
  plot.dual(flFitRes, fluorescence = "fl1", export = export)
)
```

## `r if(length(flFitRes$data$density)>1 && length(flFitRes$data$norm.fluorescence1)>1){"Graphical overview -  Normalized fluorescence 1"}`
```{r dual_plot_norm.fl1, fig.height = 24, fig.width = 20, echo=F, warning=F, message=F, results='asis', fig.align='default', eval = length(flFitRes$data$density)>1 && length(flFitRes$data$norm.fluorescence1)>1}
suppressWarnings(
  plot.dual(flFitRes, fluorescence = "norm.fl1", export = export)
)
```
...
`r if((length(flFitRes$data$density)>1 && length(flFitRes$data$fluorescence1)>1) || (length(flFitRes$data$density)>1 && length(flFitRes$data$norm.fluorescence1)>1) || (length(flFitRes$data$density)>1 && length(flFitRes$data$fluorescence2)>1) || (length(flFitRes$data$density)>1 && length(flFitRes$data$norm.fluorescence2)>1)){"\\pagebreak"}`
## `r if(length(flFit2)>1){"Fluorescence 1 fit results"} else {"Fluorescence fit results"}`
...
try(suppressWarnings(
  plot.parameter(flFitRes, param = "max_slope.linfit", export = export, out.dir = paste0(wd, "/Plots")))
)
try(suppressWarnings(
  plot.parameter(flFitRes, param = "lambda.linfit", export = export, out.dir = paste0(wd, "/Plots")))
)
try(suppressWarnings(
  plot.parameter(flFitRes, param = "dY.linfit", export = export, out.dir = paste0(wd, "/Plots")))
)
```
...
if(any(c("s") %in% control$fit.opt)){
  if(!all(is.na(res.table.fl1$lambda.spline))){
    table_spline <- data.frame("Sample|Replicate|Conc." = paste(res.table.fl1$TestId, res.table.fl1$AddId, res.table.fl1$concentration, sep = "|"),
                             ...
                             "x<sub>max</sub>" = ifelse(is.na(res.table.fl1$max_slope2.spline), round(as.numeric(res.table.fl1$x.max.spline), 2), paste0("<b>", round(as.numeric(res.table.fl1$x.max.spline), 2), "</b>", " (", round(as.numeric(res.table.fl1$x.max2.spline), 2), ")")),
...
try(suppressWarnings(
  plot.parameter(flFitRes, param = "max_slope.spline", export = export, out.dir = paste0(wd, "/Plots")))
)
try(suppressWarnings(
  plot.parameter(flFitRes, param = "lambda.spline", export = export, out.dir = paste0(wd, "/Plots")))
)
try(suppressWarnings(
  plot.parameter(flFitRes, param = "dY.spline", export = export, out.dir = paste0(wd, "/Plots")))
)
```
\clearpage
...
if(any(c("l") %in% control$fit.opt)){
  ...
  if(all(c('pdf', 'html') %in% format)){
    if (knitr::is_latex_output()) {
      ...
      #           }
      #         }
      for(i in 1:length(flFitRes[["flFit1"]][["flFittedLinear"]])){
        if(length(grep("^NA ", names(flFitRes[["flFit1"]][["flFittedLinear"]])[i]))>0 && flFitRes[["flFit1"]][["flFittedLinear"]][[i]][["fitFlag"]] == TRUE){
                  suppressWarnings(pdf(file=paste0(plotdir, "/PlotLinear_", gsub("∙", "_", gsub("\\?", "_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(flFitRes[["flFit1"]][["flFittedLinear"]])[i]))))), ".pdf"), width = 8, height = 6))
          plot.ggLinear()
          invisible(dev.off())
                 }
      }
    ...
    #         .packages = "ggplot2") %dopar% {
    #           if(flFitRes[["flFit1"]][["flFittedLinear"]][[i]][["fitFlag"]] == TRUE){
    #             pdf(file=paste0(plotdir, "/PlotLinear_", gsub("∙", "_", gsub("%", "_", gsub("\\?", "_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(flFitRes[["flFit1"]][["flFittedLinear"]])[i])))))), ".pdf"), width = 8, height = 6);   plot.ggLinear();   invisible(dev.off())
    #           }
    #         }
    for(i in 1:length(flFitRes[["flFit1"]][["flFittedLinear"]])){
        if(length(grep("^NA ", names(flFitRes[["flFit1"]][["flFittedLinear"]])[i]))==0 && flFitRes[["flFit1"]][["flFittedLinear"]][[i]][["fitFlag"]] == TRUE){
  ...
  Files_list <- list.files(path = plotdir, 
                           pattern = "^PlotLinear_",
                           full.names = TRUE)
  sort.ndx <- match(gsub("∙|>|<|\\(|\\)", "_", gsub("%", "_", gsub("\\?", "_", gsub(" ", "_", gsub("\\/", "-", gsub(" \\| ", "_", names(flFitRes$flFit1$flFittedLinear))))))), gsub(".+PlotLinear_","", gsub(".pdf", "", Files_list)))
  ...
  if ((is.na(flFitSpline$fitFlag)==TRUE)|(flFitSpline$fitFlag==FALSE)){
    ...
    if (add==TRUE){
      ...
      if (export == TRUE){
        ...
        print(p)
        suppressWarnings(suppressWarnings(grDevices::dev.off()))
      }
      if (plot == TRUE){
  ...
  if(all(c('pdf', 'html') %in% format)){
    if (knitr::is_latex_output()) {
      foreach(i = 1:length(flFitRes[["flFit1"]][["flFittedSplines"]]),
            .packages = "ggplot2") %dopar% {
              if (length(grep("^NA ", names(flFitRes[["flFit1"]][["flFittedSplines"]])[i]))==0 && flFitRes[["flFit1"]][["flFittedSplines"]][[i]][["fitFlag"]] == TRUE) {
                suppressWarnings(pdf(file = paste0(plotdir, "/PlotSpline_", gsub("∙", "_", gsub("%", "_", gsub("\\?", "_", gsub(" ", "_", gsub(
...
```{r ec50_summary, echo=F, warning=F, message=F, results='asis', eval = ec50 && length(flFitRes[["drFit1"]][["drFittedSplines"]]) > 0}
cat(paste0("_", length(flFitRes$drFit1$drTable$Test), "_ conditions were identified:\n",
           paste(colFmt(flFitRes$drFit1$drTable$Test, 'NavyBlue'), "\n", collapse = ", ")))
cat("  \n")
ec50.df <- data.frame()
for(i in 1:length(flFitRes$drFit1$drFittedSplines)){
  ec50.df <- plyr::rbind.fill(ec50.df, data.frame("Condition" = names(flFitRes$drFit1$drFittedSplines)[i],
                              "EC50" = round(flFitRes$drFit1$drFittedSplines[[i]]$parameters$EC50, 3),
                              "Response" = round(flFitRes$drFit1$drFittedSplines[[i]]$parameters$yEC50, 3))
  )
}
names(ec50.df)[3] <- paste0("Response (", flFitRes$drFit1$drFittedSplines[[1]]$parameters$test, ")")
knit_table(ec50.df, caption = "Dose-Response Analysis - Spline Fits")
```

```{r ec50_spline_plots, fig.width=7, fig.height=5, echo=F, warning=F, message=F, fig.align='center', eval = ec50 && length(flFitRes[["drFit1"]][["drFittedSplines"]]) > 0}
suppressWarnings(
  plot.drFit(flFitRes$drFit1, export = export)
)
```

```{r ec50.boot_summary, echo=F, warning=F, message=F, results='asis', eval = ec50 && length(flFitRes[["drFit1"]][["drFittedSplines"]]) > 0 && (is.numeric(control$nboot.dr) && control$nboot.dr > 0)}
boot.ec50.df <- data.frame()
  for (i in 1:length(flFitRes$drFit1$drBootSplines)){
    boot.ec50.df <-
      plyr::rbind.fill(
        boot.ec50.df,
        data.frame(
          "Condition" = flFitRes$drFit1$drTable$Test[i],
          "EC50.boot" = paste(round(as.numeric(flFitRes$drFit1$drTable$drboot.meanEC50[i]), 3),"\u00B1", round(as.numeric(flFitRes$drFit1$drTable$drboot.sdEC50[i]),3)),
          "Response.boot" = paste(round(as.numeric(flFitRes$drFit1$drTable$drboot.meanEC50y[i]), 3),"\u00B1", round(as.numeric(flFitRes$drFit1$drTable$drboot.sdEC50y[i]),3))
        )
      )
  }
...
```{r ec50_boot_plots, fig.width=6, fig.height=4.5, echo=F, warning=F, message=F, fig.align='center', eval = ec50 && length(flFitRes[["drFit1"]][["drFittedSplines"]]) > 0 && (is.numeric(control$nboot.dr) && control$nboot.dr > 0)}
for(i in 1:length(flFitRes$drFit1$drBootSplines)){
  if(flFitRes$drFit1$drBootSplines[[i]]$bootFlag == TRUE){
    plot.drBootSpline(flFitRes$drFit1$drBootSplines[[i]], width = 6, height = 4.5, export = export, out.dir = paste0(wd, "/Plots"))
  }
}
```
```{r ec50_model_summary, echo=F, warning=F, message=F, results='asis', eval = ec50 && length(flFitRes[["drFit1"]][["drFittedModels"]]) > 1}
cat(paste0("_", length(flFitRes$drFit1$drTable$Test), "_ conditions were identified:\n",
           paste(colFmt(flFitRes$drFit1$drTable$Test, 'NavyBlue'), collapse = ", ")))
cat("  \n")

ec50.df <- data.frame()
for(i in 1:length(flFitRes$drFit1$drFittedModels)){
  ec50.df <- plyr::rbind.fill(ec50.df, data.frame("Condition" = names(flFitRes$drFit1$drFittedModels)[i],
                              "K" = round(flFitRes$drFit1$drFittedModels[[i]]$parameters$K.orig, 3),
                              "y.min" = round(flFitRes$drFit1$drFittedModels[[i]]$parameters$y.min, 3),
                              "y.max" = round(flFitRes$drFit1$drFittedModels[[i]]$parameters$y.max, 3),
                              "fold change" = round(flFitRes$drFit1$drFittedModels[[i]]$parameters$fc, 3),
                              "n" = round(flFitRes$drFit1$drFittedModels[[i]]$parameters$n, 3),
                              "yEC50" = round(flFitRes$drFit1$drFittedModels[[i]]$parameters$yEC50.orig, 3))
  )
}
names(ec50.df)[7] <- paste0("Response (", flFitRes$drFit1$drFittedModels[[1]]$parameters$test, ")")
knit_table(ec50.df, caption = "Dose-Response Analysis - Spline Fits")
```

```{r ec50_model_plots, fig.width=7, fig.height=5, echo=F, warning=F, message=F, fig.align='center', eval = ec50 && length(flFitRes[["drFit1"]][["drFittedModels"]]) > 1}
for(i in 1:length(flFitRes[["drFit1"]][["drFittedModels"]])){
  try(
    suppressWarnings(
      plot.drFitModel(flFitRes$drFit1$drFittedModels[[i]], export = export)
    )
  )
}
suppressWarnings(
...
if("all" %in% mean.grp){
  if(length(flFitRes$flFit1$flFittedSplines[[1]]$spline)>1){
    ...
    suppressWarnings(
      plot.flFitRes(flFitRes, basesize = 15, export = export, log.y = logy, mean = T, out.dir = paste0(wd, "/Plots"))
    )
  }
