                            ...
                            ...)
{
  ...
  if(missing(n.ybreaks)){
    ...
    if(any(grep("y", log))){
      if(min(test) <= 0){
        log10cl.y <- round(log10(min(test[test > 0]))) - 1
        bp.y <- 10^(log10cl.y)
        ...
        if(gcFittedSpline$fitFlag2){
          ...
          if(lagtime2 < lagtime && lagtime2 > gcFittedSpline$raw.time[1]){
            ...
            if(gcFittedSpline$control$log.y.spline){
              ...
              p <- p + geom_line(aes(x = .data$time, y = .data$y), data = tangent.df2, linetype = "dashed", color = ggplot2::alpha("darkviolet", 0.85), size = lwd)
            }
